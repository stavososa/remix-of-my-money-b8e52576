...
export async function auditNomesNovaIguacu() {
  ...
  while (true) {
    const { data, error } = await supabase
      .from('vendas')
      .select('vendedor_nome,total_com_desconto,lucros_reais,nota_fiscal,familia_produto')
  ...
  const map = new Map<
    string,
    { qtd_linhas: number; faturamento: number; lucro: number; nfs: Set<string>; familias: Set<string> }
  >();

  for (const r of all) {
    const nome = (r.vendedor_nome ?? '(sem nome)').trim();
    if (!map.has(nome)) {
      map.set(nome, { qtd_linhas: 0, faturamento: 0, lucro: 0, nfs: new Set(), familias: new Set() });
    }
    ...
    if (r.familia_produto) agg.familias.add(r.familia_produto);
  }
  ...
  for (const [nome, agg] of map.entries()) {
    ...
    if (LISTA_OFICIAL.has(upper)) classificacao = 'OFICIAL';
      ...
      isCanalExterno(nome) ||
      Array.from(agg.familias).some((f) => isCanalExterno(null, f))
    ) classificacao = 'CANAL_EXTERNO';
